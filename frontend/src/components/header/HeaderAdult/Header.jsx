...
export default function Header({  isKids, isWhite,onExploreClick, cartItems, removeItemFromCart }) { 
  ...
  return (
    <div>
       <header className={`headerStructure ${isKids ? 'kids-version' : ''} ${isWhite ? 'white-version' : ''}`}>
        <div className={`logo2 ${isKids ? 'kids-logo' : ''} ${isWhite ? 'white-logo' : ''}`}>
        ...
        <div className={`listHeader ${isMenuOpen ? 'open' : ''}`}>  {/* Clase 'open' para abrir el menú */}
          {linkHeaders.map((link, index) => (
            <Link  
              key={index}
              to={link.url}  
              className={`headerLink ${isKids ? 'kids-link' : ''} ${isWhite ? 'white-link' : ''}`}
        ...
        <div className="ctaList">
          {!isLoggedIn ? (
            <>
              <div className={`login ${isKids ? 'login-kids' : ''} ${isWhite ? 'login-white' : ''}`}>
                <button onClick={handleLoginClick}>Inicia Sesión</button>
              </div>
              <div className={`Register ${isKids ? 'register-Kids' : ''} ${isWhite ? 'register-white' : ''}`}>
        ...
        <div className="hamburger-menu" onClick={toggleMenu}>
          <span className={`hamburger-icon ${isKids ? 'hamburguer-kids' : ''} ${isWhite ? 'hamburguer-white' : ''}`}>&#9776;</span>  {/* Icono del menú */}
        </div>
      </header>
    </div>
  );
